...
/*分析类容*/
int analyze_content(const u_char * pkt, struct datapkt *data, char *msg)
{
	//不需要过滤直接跳过
	...
	const u_char *p = NULL;
	int iplen = int(data->iph->ihl) * 4;
	int tcplen = 0;
	p = pkt + 14 + iplen;

	char temp[10240] = { 0 };
	if (strcmp(data->pktType, "UDP") == 0)
	{
		memcpy(temp, p+8, (data->udph->len)-8);
		if (strstr(temp, msg) > 0)
			return 1;
	}
	else if (strcmp(data->pktType, "TCP") == 0)
	{
		tcplen = int(data->tcph->doff * 4);
		memcpy(temp, p + tcplen, (data->iph->tlen) - iplen - tcplen);
		if (strstr(temp, msg) > 0)
			return 1;
	}
